...
const cart = {
    ...
    createSandwichCard(sandwich) {
        ...
        sandwichCard.className = this.selectedSandwich.id === sandwich.id ? 'm-3 card border-primary' : 'm-3 card'
    ...
    async saveSelectedSandwich() {
        ...
        fetch(`http://localhost:3001/cart/${this.selectedSandwich.id}`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify(this.selectedSandwich)
        })
    },
    ...
    async duplicateSandwich(sandwich) {
        ...
        this.items.push(newSandwich)
        this.selectSandwich(newSandwich)
    },
